...
def buka_jendela_unduh():
    ...
    label_kat = ctk.CTkLabel(panel_atas_unduh, text="Simpan ke:")
    ...
    def ambil_data_pencarian(keyword, halaman, label_loading):
        try:
            ...
            app.after(0, lambda: label_loading.configure(text="Gagal terhubung!", text_color="red"))
    ...
    def render_baris_hasil(nama_asli, url_download, url_halaman):
        ...
        btn_preview = ctk.CTkButton(
            ...
            command=lambda: webbrowser.open(url_halaman)
        )
    ...
    def proses_unduh(nama_bersih, url_download, btn_dl):
        ...
        try:
            ...
            # Menggunakan stream=True agar proses download lebih stabil
            ...
            if response.status_code == 200:
                ...
                # Jika masih gagal, Terminal akan mencetak kode erornya agar kita tahu alasannya
                raise Exception(f"Ditolak server (Kode HTTP {response.status_code})")
            ...
            if os.path.exists(path_wav):
                ...
                if os.path.exists(path_mp3): os.remove(path_mp3)
                raise Exception("Konversi FFmpeg gagal")
...
def toggle_favorite(nama_file, path_lengkap, btn):
    ...
    # Ambil teks dari form pencarian lokal jika ada
    keyword = entry_cari_lokal.get() if 'entry_cari_lokal' in globals() else ""

    if dropdown_kategori.get() == "⭐ Favorit" and not is_fav:
        btn.destroy()
        if not kategori_dict.get("⭐ Favorit"):
            dropdown_kategori.set("Semua Suara")
            ganti_kategori("Semua Suara", keyword)

def ganti_kategori(pilihan_kategori, keyword=""):
    ...
    if keyword.strip():
        ...
        daftar_file = [f for f in daftar_file if keyword_lower in f[0].lower()]
...
def muat_ulang_suara():
    ...
    keyword_saat_ini = entry_cari_lokal.get() if 'entry_cari_lokal' in globals() else ""
    ...
    if semua_file:
        ...
        if kategori_saat_ini in list_kategori:
            ...
            ganti_kategori(kategori_saat_ini, keyword_saat_ini)
        else:
            target = "⭐ Favorit" if "⭐ Favorit" in list_kategori else "Semua Suara"
            dropdown_kategori.set(target)
            ganti_kategori(target, keyword_saat_ini)
    else:
...
btn_unduh.grid(row=0, column=3, padx=10, pady=10) 

panel_atas.grid_columnconfigure(4, weight=1)

btn_panic = ctk.CTkButton(
    panel_atas, text="🛑 STOP", fg_color="#c62828", hover_color="#9e1c1c",
    font=("Arial", 12, "bold"), width=80, command=stop_all_sounds
)
btn_panic.grid(row=0, column=5, padx=15, pady=10, sticky="e")

# --- PANEL DROPDOWN KATEGORI & PENCARIAN LOKAL ---
...
# Pilihan Kategori
dropdown_kategori = ctk.CTkOptionMenu(
    panel_kategori, dynamic_resizing=False, width=150, 
    command=lambda c: ganti_kategori(c, entry_cari_lokal.get())
)
dropdown_kategori.pack(side="left")

# Label Bantuan
label_bantuan = ctk.CTkLabel(panel_kategori, text="(Klik Kanan: ⭐)", font=("Arial", 12, "italic"), text_color="gray")
label_bantuan.pack(side="left", padx=10)

# Kotak Pencarian Lokal
entry_cari_lokal = ctk.CTkEntry(panel_kategori, placeholder_text="🔍 Cari nama suara...", width=160)
entry_cari_lokal.pack(side="right", padx=(0, 10))
# Memicu filter secara real-time setiap kali pengguna mengetik huruf
entry_cari_lokal.bind("<KeyRelease>", lambda e: ganti_kategori(dropdown_kategori.get(), entry_cari_lokal.get()))
